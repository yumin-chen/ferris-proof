...
use blake3::Hasher;
use syn::{parse_file, File, Item, ItemFn, ItemStruct, ItemEnum, ItemMod, Attribute};
use quote::ToTokens;
...
impl CacheKey {
    ...
    pub fn new(target: &crate::verification::Target, layer: Layer, config_hash: &str) -> Result<Self> {
    ...
    fn compute_content_hash(target: &crate::verification::Target) -> Result<ContentHash> {
        ...
        match target {
            crate::verification::Target::RustFile(path) => {
                let ast = parse_file(&std::fs::read_to_string(path)
    ...
    fn normalize_function(item_fn: &ItemFn) -> ItemFn {
        ...
        let attrs: Vec<Attribute> = item_fn.attrs
            .iter()
            .filter(|attr| {
                // Keep verification attributes, remove others like #[cfg(test)]
                attr.path().is_ident("verification") ||
                attr.path().segments.first().map(|seg| seg.ident == "cfg").unwrap_or(false) == false
            })
            .cloned()
            .collect();
    ...
    fn normalize_struct(item_struct: &ItemStruct) -> ItemStruct {
        // Remove attributes that don't affect verification
        let attrs: Vec<Attribute> = item_struct.attrs
            .iter()
    ...
    fn normalize_enum(item_enum: &ItemEnum) -> ItemEnum {
        // Remove attributes that don't affect verification
        let attrs: Vec<Attribute> = item_enum.attrs
            .iter()
            .filter(|attr| attr.path().is_ident("verification"))
            .cloned()
            .collect();
    ...
    fn normalize_module(item_mod: &ItemMod) -> ItemMod {
        // Remove attributes that don't affect verification
        let attrs: Vec<Attribute> = item_mod.attrs
            .iter()
            .filter(|attr| attr.path().is_ident("verification"))
            .cloned()
            .collect();
    ...
    fn get_tool_version_with_arg(tool_name: &str, version_arg: &str) -> Result<String> {
        ...
        match output {
            Ok(result) => {
                ...
                let version_text = if !stdout.trim().is_empty() { stdout } else { stderr };
                ...
                if !version.is_empty() {
                    ...
                    Err(anyhow!("Could not parse version from output: {}", version_line))
                }
            }
            Err(e) => {
                Err(anyhow!("Failed to execute tool {}: {}", tool_name, e))
            }
        }
    }
    ...
    fn get_rustc_commit_hash() -> Result<String> {
        ...
        match output {
            ...
            Ok(result) => {
                Err(anyhow!("rustc returned non-zero exit code: {}", result.status))
            }
            Err(e) => {
                Err(anyhow!("Failed to execute rustc: {}", e))
            }
        }
    }
}
...
impl PersistentStorage {
    ...
    fn load_all(&self) -> Result<HashMap<CacheKey, CacheEntry>> {
        ...
        fn visit_dir(dir: &Path, entries: &mut HashMap<CacheKey, CacheEntry>, storage: &PersistentStorage) -> Result<()> {
            for entry in std::fs::read_dir(dir)? {
                ...
                } else if path.is_file() && path.extension().and_then(|s| s.to_str()) == Some("cache") {
