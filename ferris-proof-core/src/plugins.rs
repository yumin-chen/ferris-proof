...
impl PluginManager {
    ...
    pub fn register_plugin(&mut self, plugin: Box<dyn VerificationPlugin>) -> Result<()> {
        ...
        if !self.version_checker.is_compatible(&version_range) {
            return Err(anyhow!(
                ...
                name,
                version,
                self.version_checker.current_version
            ));
        }
    ...
    pub fn plugins_for_technique(
        &self,
        technique: &Technique,
    ) -> Vec<Arc<RwLock<Box<dyn VerificationPlugin>>>> {
        self.plugins
            .values()
            .filter(|plugin| {
                ...
                Ok(plugin) => {
                    let validation_result = match plugin.check_availability() {
                        Ok(tool_info) => {
                            ...
                            let version_validation =
                                self.version_checker.validate_tool_version(&tool_info, None);
    ...
    pub async fn verify(
        ...
        if plugins.is_empty() {
            return Err(anyhow!(
                "No plugins available for technique: {:?}",
                technique
            ));
        }
        ...
        if let Some(plugin_arc) = plugins.first() {
            ...
            Err(anyhow!(
                "No plugin available for technique: {:?}",
                technique
            ))
        }
    }
    ...
    pub fn list_plugins(&self) -> Vec<PluginMetadata> {
        ...
        for registration in self.plugin_registry.registered_plugins.values() {
            if !self
                .plugin_registry
                .loaded_plugins
                .contains(&registration.metadata.name)
            {
...
impl VersionChecker {
    ...
    fn validate_tool_version(
        &self,
        tool_info: &ToolInfo,
        expected_range: Option<&VersionRange>,
    ) -> Result<()> {
