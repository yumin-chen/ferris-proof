use ferris_proof_config::{ConfigManager, Config, parse_verification_attributes};
...
"#;
    ...
    assert!(effective_config.enabled_techniques.contains(&Technique::FormalSpecs));
}
...
"#;
    ...
    assert!(config2.profile.enabled_techniques.contains(&Technique::SessionTypes));
    assert!(config2.profile.enabled_techniques.contains(&Technique::PropertyTests));
}
