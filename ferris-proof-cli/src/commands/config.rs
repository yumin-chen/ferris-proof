...
async fn validate_configuration() -> Result<i32> {
    ...
    match ConfigManager::from_project_root(&current_dir) {
        Ok(config_manager) => match config_manager.validate() {
            Ok(()) => {
...
async fn show_file_configuration(file_path: &Path) -> Result<i32> {
    println!(
        "{}",
        format!("Configuration for file: {}", file_path.display()).cyan()
    );
...
async fn show_project_configuration() -> Result<i32> {
    ...
    if !config_file.exists() {
        println!(
            "✗ {}",
            "No ferrisproof.toml found in current directory".red()
        );
        println!(
            "  Run {} to initialize a project",
            "ferris-proof init".cyan()
        );
        return Ok(1);
    }
    ...
    let config_content =
        std::fs::read_to_string(&config_file).context("Failed to read ferrisproof.toml")?;

    let root_config: Config =
        toml::from_str(&config_content).context("Failed to parse ferrisproof.toml")?;
...
fn display_config(config: &Config) {
    ...
    println!(
        "  Enforcement: {}",
        format!("{:?}", config.profile.enforcement).green()
    );
    ...
    if let Some(ref proptest_config) = config.tools.proptest {
        ...
        if let Some(max_shrink_iters) = proptest_config.max_shrink_iters {
            println!(
                ...
                max_shrink_iters.to_string().green()
            );
        }
    }
    ...
    println!(
        "  Cache Enabled: {}",
        config.features.cache_enabled.to_string().green()
    );
    println!(
        "  Parallel Execution: {}",
        config.features.parallel_execution.to_string().green()
    );
    println!(
        "  Generate Reports: {}",
        config.features.generate_reports.to_string().green()
    );

    println!("\n{}", "Thresholds:".yellow());
    println!(
        "  Max Verification Time: {}s",
        config.thresholds.max_verification_time.to_string().green()
    );
    println!(
        "  Max Memory Usage: {} bytes",
        config.thresholds.max_memory_usage.to_string().green()
    );
    println!(
        "  Cache TTL: {}s",
        config.thresholds.cache_ttl.to_string().green()
    );

    println!("\n{}", "CI Configuration:".yellow());
    println!(
        "  Fail on Violations: {}",
        config.ci.fail_on_violations.to_string().green()
    );
    println!(
        "  Generate Artifacts: {}",
        config.ci.generate_artifacts.to_string().green()
    );
    println!(
        "  Upload Reports: {}",
        config.ci.upload_reports.to_string().green()
    );
}
...
fn display_effective_config(config: &ferris_proof_config::manager::EffectiveConfig) {
    ...
    println!(
        "  Enforcement: {}",
        format!("{:?}", config.enforcement).green()
    );
    println!("  Enabled Techniques:");
