use ferris_proof_core::{
    types::*,
    plugins::{VerificationPlugin, PluginMetadata, VersionRange, ToolInfo, VerificationInput, VerificationOutput, StructuredResult, PerformanceMetrics},
    verification::Target,
};
...
"#;
    ...
    fn run_proptest(&self, target: &Target, config: &VerificationInput) -> Result<VerificationOutput> {
        ...
        match target {
            Target::RustFile(path) => {
                ...
                let temp_dir = config.context.cache_dir.join(format!("proptest_{}", uuid::Uuid::new_v4()));
                ...
                cmd.env("PROPTEST_CASES", config.config.tool_config.get("cases").and_then(|v| v.as_u64()).unwrap_or(1000).to_string());
                cmd.env("PROPTEST_MAX_SHRINK_ITERS", config.config.tool_config.get("max_shrink_iters").and_then(|v| v.as_u64()).unwrap_or(10000).to_string());
                ...
                let violations = if structured_result.status == Status::Error {
                    vec![
                        Violation {
                            ...
                            suggestion: Some("Check the test output for specific failure details".to_string()),
                ...
                Ok(VerificationOutput {
                    ...
                    metrics: VerificationMetrics {
                        ...
                        test_cases_executed: structured_result.statistics.get("test_cases_executed").and_then(|v| v.as_u64()).unwrap_or(0) as u32,
                    },
                })
            }
...
impl VerificationPlugin for ProptestPlugin {
    ...
    fn check_availability(&self) -> Result<ToolInfo> {
        ...
        match (cargo_result, rustc_result) {
            (Ok(cargo_output), Ok(rustc_output)) if cargo_output.status.success() && rustc_output.status.success() => {
            ...
            (Err(cargo_err), _) => {
                Err(anyhow!("Cargo not found: {}", cargo_err))
            }
            (_, Err(rustc_err)) => {
                Err(anyhow!("Rustc not found: {}", rustc_err))
            }
        }
    }
    ...
    fn metadata(&self) -> PluginMetadata {
        PluginMetadata {
            ...
            homepage: Some("https://altsysrq.github.io/proptest-book/proptest/index.html".to_string()),
    ...
    fn initialize(&mut self, config: &serde_json::Value) -> Result<()> {
        ...
        info!("Proptest plugin initialized with tool: {:?}", self.tool_path);
        Ok(())
    }
