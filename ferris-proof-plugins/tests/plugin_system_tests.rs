use ferris_proof_plugins::{TlaPlusPlugin, ProptestPlugin, SandboxedExecutor};
use ferris_proof_plugins::sandbox::{NetworkPolicy, ResourceLimits};
use ferris_proof_core::plugins::{VerificationPlugin, PluginManager, ValidationStatus};
...
mod plugin_system_tests {
    ...
    fn test_plugin_loading_and_version_compatibility() {
        ...
        assert!(plugin_metadata.is_some(), "Plugin metadata should be available");
        ...
        assert!(load_result.is_ok(), "Proptest plugin should load successfully");
        
        // Verify both plugins are listed
        let all_plugins = plugin_manager.list_plugins();
        assert!(all_plugins.len() >= 2, "Should have at least 2 plugins registered");
        
        let plugin_names: Vec<String> = all_plugins.iter().map(|p| p.name.clone()).collect();
        assert!(plugin_names.contains(&plugin_name), "TLA+ plugin should be in list");
        assert!(plugin_names.contains(&proptest_name), "Proptest plugin should be in list");
    }
    ...
    fn test_tool_discovery_and_availability() {
        ...
        plugin_manager.register_plugin(tla_plugin).expect("TLA+ plugin should register");
        plugin_manager.register_plugin(proptest_plugin).expect("Proptest plugin should register");
        
        // Validate tools
        let validation_results = plugin_manager.validate_tools().expect("Tool validation should succeed");
        
        assert!(!validation_results.is_empty(), "Should have validation results");
        
        for result in &validation_results {
            // Each plugin should have a validation result
            assert!(!result.plugin_name.is_empty(), "Plugin name should not be empty");
            
            match result.status {
                ValidationStatus::Valid => {
                    assert!(result.tool_info.is_some(), "Valid tools should have tool info");
                    let tool_info = result.tool_info.as_ref().unwrap();
                    assert!(tool_info.available, "Valid tools should be marked as available");
                    assert!(!tool_info.capabilities.is_empty(), "Valid tools should have capabilities");
                }
                ValidationStatus::Unavailable => {
                    // Tool not available - this is acceptable in test environments
                    assert!(!result.issues.is_empty(), "Unavailable tools should have issues listed");
                }
                ValidationStatus::VersionIncompatible => {
                    // Version incompatible - should have tool info but marked as incompatible
                    assert!(!result.issues.is_empty(), "Incompatible tools should have issues listed");
                }
                ValidationStatus::Error => {
                    // Error occurred during validation
                    assert!(!result.issues.is_empty(), "Error status should have issues listed");
                }
            }
        }
    }
    ...
    fn test_plugin_metadata_and_capabilities() {
        ...
        assert!(tla_metadata.techniques.contains(&ferris_proof_core::types::Technique::FormalSpecs));
        assert!(tla_metadata.techniques.contains(&ferris_proof_core::types::Technique::ModelChecking));
        ...
        assert!(proptest_metadata.techniques.contains(&ferris_proof_core::types::Technique::PropertyTests));
        ...
        assert!(tla_versions.min.is_some() || tla_versions.max.is_some() || tla_versions.requires_exact.is_some());
        
        let proptest_versions = proptest_plugin.supported_versions();
        assert!(proptest_versions.min.is_some() || proptest_versions.max.is_some() || proptest_versions.requires_exact.is_some());
    }
    ...
    fn test_plugin_initialization_and_cleanup() {
        ...
        assert!(proptest_cleanup.is_ok(), "Proptest plugin cleanup should succeed");
    }
    ...
    async fn test_sandboxed_execution_with_resource_limits() {
        ...
        match result {
            Ok(output) => {
                ...
                assert!(output.stdout.contains("Hello, World!"), "Output should contain expected text");
        ...
        match result {
            Ok(output) => {
                ...
                if output.timeout_occurred {
                    assert_ne!(output.exit_code, 0, "Timed out command should have non-zero exit code");
                    assert!(output.stderr.contains("timeout"), "Error should mention timeout");
                } else {
    ...
    fn test_plugin_manager_multiple_plugins() {
        ...
        let formal_plugins = plugin_manager.plugins_for_technique(&ferris_proof_core::types::Technique::FormalSpecs);
        assert!(!formal_plugins.is_empty(), "Should have plugins for formal specs");
        
        let property_plugins = plugin_manager.plugins_for_technique(&ferris_proof_core::types::Technique::PropertyTests);
        assert!(!property_plugins.is_empty(), "Should have plugins for property tests");
    ...
    fn test_resource_limits_configuration() {
        ...
        assert!(true, "Resource limits should be configurable");
        
        // Test default limits
        let default_limits = ResourceLimits::default();
        assert!(default_limits.max_memory > 0, "Default memory limit should be positive");
        assert!(default_limits.max_cpu_time > 0, "Default CPU time limit should be positive");
        assert!(default_limits.max_file_descriptors > 0, "Default file descriptor limit should be positive");
        assert!(default_limits.max_processes > 0, "Default process limit should be positive");
        assert!(default_limits.max_file_size > 0, "Default file size limit should be positive");
    }
    ...
    fn test_network_policy_configuration() {
        ...
        assert_eq!(policy3, policy4, "Identical allowlist policies should be equal");
        
        let policy5 = NetworkPolicy::Unrestricted { user_consent: true };
        let policy6 = NetworkPolicy::Unrestricted { user_consent: true };
        assert_eq!(policy5, policy6, "Identical unrestricted policies should be equal");
    }
}